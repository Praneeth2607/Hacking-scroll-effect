...
const dissolveColumn = Math.ceil(window.innerWidth / dissolveCellSize);
...
const dissolveCharacters = "ZXCVBNMASDFGHJKLQWERTYUIOP;',.[]-=_+:?<>!@#$%^&*()";
...
for (let row = 0; row < dissolveRows; row++) {
    for (let col = 0; col < dissolveColumn; col++) {
...
const cellScatterOffset = dissolveCells.map(
    (cell) => (hashFromPosition(cell.row,cell.col,269.3)-0.5) * dissolveScatterIntensity,
);
...
function updateImageClipPaths(scrollProgress, travelRange){
    for (let i = 0; i<totalTransitions; i++){
        ...
        const remappedPosition = -dissolveSpreadAbove + segmentProgress * travelRange;
        const clipPercent = gsap.utils.clamp(0, 100, remappedPosition * 100);

        stackedImages[i].style.clipPath = `polygon(0% ${clipPercent}%, 100% ${clipPercent}% ,100% 100%, 0% 100%)`;

    }
}

function hideAllDissolveCells() {
    dissolveCellElements.forEach(el => {
        el.style.visibility = "hidden";
    });
}

function updateDissolveBand(bandCenterY) {
    dissolveCells.forEach((cell, i) => {
        ...
        const normalizedDistance = scatteredDistance >= 0 ? scatteredDistance / dissolveSpreadBelow : Math.abs(scatteredDistance) / dissolveSpreadAbove;
...
ScrollTrigger.create({
    ...
    onUpdate: (self) => {
        ...
        const transitionProgress = gsap.utils.clamp(
            0,1,rawPosition-currentTransition,
        );
        ...
        const bandCenterY = -dissolveSpreadAbove + transitionProgress * totalTravelRange;
